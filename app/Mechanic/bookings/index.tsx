// app/mechanic/bookings/index.tsx
...
// COMPREHENSIVE TANZANIAN LOCATIONS DATABASE (Same as MapViewModal)
const TANZANIA_LOCATIONS: Record<string, { lat: number; lng: number }> = {
  // DAR ES SALAAM REGION (40+ locations)
...
const BookingsManagement = () => {
  ...
  const fetchBookings = useCallback(async () => {
    try {
      ...
      const mappedBookings = data.map((booking: any) => ({
        ...
        garage_name: booking.garage_name || 'Unknown Garage',
        garage_city: booking.garage_city || '',
        service_name: booking.service_name || 'Custom Service',
      ...
      const newStats = {
        ...
        pending: mappedBookings.filter((b: BookingType) => b.status === 'pending').length,
        confirmed: mappedBookings.filter((b: BookingType) => b.status === 'confirmed').length,
        in_progress: mappedBookings.filter((b: BookingType) => b.status === 'in_progress').length,
        completed: mappedBookings.filter((b: BookingType) => b.status === 'completed').length,
        cancelled: mappedBookings.filter((b: BookingType) => b.status === 'cancelled').length,
      };
      setStats(newStats);
    } catch (error) {
  ...
  const geocodeAddress = async (address: string, bookingId?: number): Promise<MapCoordinates | null> => {
    try {
      ...
      // Check comprehensive Tanzanian locations database
      ...
      for (const format of geocodeFormats) {
        try {
          ...
          console.warn(`Geocoding format failed:`, formatError);
          continue;
        }
      }
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(booking =>
        ...
        booking.mobile_number.includes(query) ||
  ...
  const renderBookingItem = ({ item }: { item: BookingType }) => (
    <View style={styles.bookingCard}>
      <View style={styles.bookingHeader}>
        <View style={styles.customerInfo}>
          ...
          <Text style={styles.garageInfo}>
            {item.garage_name} • {item.garage_city || 'No city'}
          </Text>
        </View>
        <View style={styles.bookingMeta}>
          <Text style={styles.bookingNumber}>#{item.booking_number}</Text>
          <Text style={styles.createdDate}>{formatCreatedDate(item.created_at)}</Text>
        </View>
      </View>
      ...
      <View style={styles.actionButtons}>
        ...
        {item.location && (
          <TouchableOpacity
            style={[styles.mapButton, geocodingProgress[item.id] && styles.mapButtonDisabled]}
            onPress={() => handleShowMap(item)}
            disabled={geocodingProgress[item.id]}
          >
            {geocodingProgress[item.id] ? (
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerSubtitle}>Manage customer appointments</Text>
        </View>
      </View>
        ...
        >
          {statusOptions.map((option) => (
            ...
            >
              ...
              <View style={[
                styles.filterCount,
                statusFilter === option.value && styles.filterCountActive
              ]}>
        ...
        {loading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtitle}>
              Customer bookings will appear here once they book your services
            </Text>
          </View>
            ...
            <View style={styles.modalHeader}>
              <View>
                ...
                <Text style={styles.modalSubtitle}>
                  #{selectedBooking?.booking_number} • {formatCreatedDate(selectedBooking?.created_at || '')}
                </Text>
              </View>
              <TouchableOpacity 
                  ...
                  <View style={styles.section}>
                    ...
                    <View style={styles.customerCard}>
                      ...
                      <View style={styles.customerInfoLarge}>
                        <Text style={styles.customerNameLarge}>{selectedBooking.full_name}</Text>
                        <Text style={styles.customerPhone}>{selectedBooking.mobile_number || 'No phone'}</Text>
                  ...
                  <View style={styles.section}>
                    ...
                    <View style={styles.serviceCard}>
                      <View style={styles.serviceHeader}>
                        ...
                        <Text style={styles.serviceNameLarge}>{selectedBooking.service_name}</Text>
                      </View>
                      <View style={styles.serviceDetails}>
                        <View style={styles.serviceDetail}>
                          <Ionicons name="business-outline" size={18} color="#6b7280" />
                          <Text style={styles.serviceDetailText}>{selectedBooking.garage_name}</Text>
                        </View>
                        <View style={styles.serviceDetail}>
                          <Ionicons name="location-outline" size={18} color="#6b7280" />
                          <Text style={styles.serviceDetailText}>{selectedBooking.garage_city || 'No city'}</Text>
                        </View>
                      </View>
                    </View>
                  </View>
                      ...
                      <View style={styles.locationCard}>
                        <View style={styles.locationRow}>
                          ...
                          <View style={styles.locationInfo}>
                            ...
                            <Text style={styles.locationAddress}>{selectedBooking.location}</Text>
                          </View>
                        </View>
                      </View>
                    </View>
                  ...
                  <View style={styles.section}>
                    ...
                    <View style={styles.statusCard}>
                      ...
                      <Text style={styles.statusDescription}>
                        {selectedBooking.status === 'pending' && 'Awaiting your confirmation'}
                        {selectedBooking.status === 'confirmed' && 'Confirmed and scheduled'}
                        {selectedBooking.status === 'in_progress' && 'Service currently in progress'}
                        {selectedBooking.status === 'completed' && 'Service completed successfully'}
